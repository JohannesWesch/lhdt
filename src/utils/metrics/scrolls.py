...
def rouge_config():
    def transform_result_fn(output):
        return {
            key: (value[0] if isinstance(value, list) else value).fmeasure * 100
            for key, value in output.items()
        }

    def transform_aggregated_result_fn(output):
        output.update({
            "geometric_mean": (output["rouge1"] * output["rouge2"] * output["rougeL"]) ** (1.0 / 3.0)
        })
        return output

    def transform_single_input_fn(text):
        return rouge_postprocess_text(text)
    ...
    return {
        ...
        "transform_single_input_fn": transform_single_input_fn,
        "transform_result_fn": transform_result_fn,
        "transform_aggregated_result_fn": transform_aggregated_result_fn
    }
...
def default_transform_result_fn(x):
    return x

def default_transform_aggregated_result_fn(x):
    return x
...
class Scrolls(MetricBase):
    def __init__(self, *args, **kwargs):
        ...
        self._compute_helper_kwargs_fn = {
            "rouge": rouge_config,
            "exact_match": exact_match_config,
            "f1": f1_config,
        }
...
def _compute_helper(
    ...
    if transform_result_fn is None:
        transform_result_fn = default_transform_result_fn
        do_transform_result = False
    else:
        do_transform_result = True

    if transform_aggregated_result_fn is None:
        transform_aggregated_result_fn = default_transform_aggregated_result_fn
